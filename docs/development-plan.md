...
### 実機検証済み
- 本物のChromeで拡張サイドパネルが起動し、`chrome.debugger` のクリックを弥生の画面が受け付けることを確認。
- 「契約管理を開いて内容を報告して」が、ページ本文を読んで done で報告するところまで動作。
  （初期はページ本文をClaudeに渡しておらずループしたが、`page.text` を渡すことで解消）

---

## 現在の実装状態（2026-07-04時点）

拡張は「AIループ一本」から、**決定的レシピ＋AIループのハイブリッド**へ発展した。
読み取りに加え、**書き込み（取引の登録）も確認ゲート付きで実機動作**するところまで到達した。
設計の詳細は [recipe-design.md](recipe-design.md) を参照。

### 完了済み

- [x] **エージェントループ（AI）**: `background.js` が DOM＋本文を抽出 →
      `/api/agent/next`（Claude）→ `chrome.debugger` で trusted 操作、を done まで繰り返す。
- [x] **画面モデル / 現在地特定**（`screens.js`, recipe-design.md §3）
  - 弥生は **MPA**（画面ごとに安定URL）。`host+path` を一意キーに、最長一致で現在地を判定。
  - 入口URLは `service_id=shinkoku` 付き（無いとポータルに飛ぶ）。ログインは割り込み画面として最優先判定。
  - 未定義ページはタイトル「製品 - 画面名」から画面名を自動導出（常に名前が出る）。
- [x] **画面カタログの自動取得**: 弥生を普通に使うだけで訪れた画面の path/title/URL を蓄積・書き出し。
      人力で1画面ずつ定義しなくてもマップが手に入る。実データ19画面で検証済み。
- [x] **レシピ・ルーター（Phase 1）**（`recipes.js`, recipe-design.md §4-6）
  - 指示にマッチすれば `goToScreen` で目的画面へ **URL直行**（別画面からでも1発移動を確認）。
  - 判断が要る操作は AIループにフォールバック。トリガーは具体→汎用の順で評価。
- [x] **Wijmo フィールドへの入力対応**（2026-07-02）
  - 日付欄等は Wijmo コントロール（`type=tel` の生inputに見えるが JS が1打ごとに再パース）で、
    キー入力・`insertText` は横取りされ値が壊れていた。
  - `world:"MAIN"` で `window.wijmo` の Control を取得し `control.value` を直接セット。
    取れない欄は native setter＋イベント → 従来キー入力へ段階フォールバック。実機で検証済み。
- [x] **レポート期間指定の決定的化**（2026-07-02）
  - 各レポート共通の期間UI（`#SearchStartDate`/`#SearchEndDate`、日指定モード）を、指示から
    期間を導出（`dates.js` の `parseRange`：「6月」「先月」「6/1〜6/30」「第1四半期」等）して決定的にセット。
    期間欄を変えると自動反映（表示ボタン無し）。全7レポートに `periodReport()` で展開。
- [x] **書き込みレシピ（かんたん取引入力の登録）＋確認ゲート**（2026-07-04）— 書き込み側の最初の実機成功
  - 方式は **AI抽出 → 決定的入力**。純AIループはこの画面（Wijmo＋タブ＋破棄ダイアログ）で
    要素番号の推測が不安定だったため、Claude は指示文の**構造化抽出だけ**を担い、入力は決定的に行う。
    - `extract_dealing.py` ＋ `/api/agent/extract-dealing`：指示 →
      `{区分, 取引日, 科目, 取引手段, 金額, 摘要?, 取引先?}`（ツールのプロパティ名はASCII必須）。
    - レシピ `r-register-dealing`：区分タブ→各欄→登録を、**ラベル基点**（`resolveFieldSelector`）で
      決定的に入力。全ステップ `mutates` で、実行前に確認ゲートへ一括提示。
  - **KamokuBox（科目・取引手段コンボ）の候補選択**：`indexOf`/`selectedIndex`（`displayMemberPath="label"`）で
    部分一致から確定（「三井住友」→「クレジットカード（三井住友VISAカード）」）。実機で候補110/27件を確認。
  - **更新系クリックの確認ゲート**（AIループ側）：登録/保存/更新/削除ボタンの押下直前に、フォーム値を
    提示して承認を待つ安全網（決定的パスが失敗してAIに落ちた場合の保険）。

### ロードマップ（今後）

- [ ] **Phase 2 — 画面・遷移・レシピの拡充**
  - カタログを元に対象画面／遷移／レシピのトリガーを増やす。
  - グリッド行の選択（rowSelector / 仮想スクロール）への対応（recipe-design.md §10.1）。
- [ ] **Phase 3 — 書き込みレシピの拡充**（登録＋確認ゲートの基盤は完了）
  - 区分タブ（収入/振替）のクリックを確実化（現状は支出デフォルトのみ検証。他区分＋破棄ダイアログ対策）。
  - 摘要・取引先つきの登録、複数明細、仕訳入力側の書き込みへ展開。
  - 「読み取り・確認系ではユーザー指示が無い限り書き込まない」制約は継続。最後の保存は人間。
- [ ] **堅牢性**: ループ検知・リトライ・要素未検出時の停止/委譲、dry-run とトリガー優先度の lint（§10.3）。
- [ ] **開発フロー**: WSLで編集する `extension/` を Cドライブ側（Chromeが読むコピー）へ同期する手順の自動化。
- [ ] 音声入力・プロンプトテンプレート等の UX。
