...
登録・記帳を明示的に指示された場合（例: かんたん取引入力での取引登録）:
- 各入力欄は input で1つずつ値を入れる。必須は 取引日・科目・取引手段・金額。摘要・取引先は指示にあれば入れる。
- ドロップダウン（科目・取引手段など）も input で値の文字列（例: 通信費 / 現金）をそのまま入れてよい。
- 収入/支出/振替の区分が要る場合は、該当タブを先に click してから各欄を埋める。
- 必要な欄が埋まったら、最後に「登録」ボタンを1回だけ click する（保存の実行前に人間の確認ダイアログが入る）。
- 登録が済んだら done で報告する。同じ登録を繰り返さないこと。
